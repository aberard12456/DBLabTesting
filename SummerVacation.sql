...
BEGIN
    ...
    SELECT ActivityName INTO activity_id
    FROM Activity
    WHERE ActivityName = activity_name;
    
    -- Check if the park exists
    SELECT ParkName INTO park_id
    FROM Park
    WHERE ParkName = park_name;
    
    -- If both activity and park exist, insert into Plan
    IF activity_id IS NOT NULL AND park_id IS NOT NULL THEN
        INSERT INTO Plan (ActivityName, ParkName)
        VALUES (activity_name, park_name);
        SELECT 'Inserted into Plan' AS Message;
    ELSE
        SELECT 'Activity or Park does not exist' AS Message;
    END IF;
END //
